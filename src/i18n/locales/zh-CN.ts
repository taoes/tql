const raw = {
  ...
  aiChat: {
    ...
    systemPrompt:
      ...
      "\\n6. **输出格式规则**：当用户明确要求执行增删改查操作（SELECT / INSERT / UPDATE / DELETE）时，你**只能**输出纯 SQL 语句，不要添加任何解释、说明或 Markdown 代码块标记（如 \\`\\`\\`sql）。当用户询问数据库信息、表结构、字段含义等非操作性问题时，正常输出文字说明。判断标准：用户意图是操作数据 → 只输出 SQL；用户意图是了解信息 → 输出文字。",
  },
  workspace: {
    aiTab: "AI 对话",
    sqlTab: "SQL 结果 {n}",
